 ...
 * $Id: tok.c 4 2007-08-27 13:11:03Z xtimor $
 *
 */
...
{
    const char *beg_tok;
    const char *end_buf = dst_buff + buff_sz;
    ...
    for(; *format && dst_buff < end_buf; ++format)
    {
        switch(tok_type)
        {
        ...
        case NMEA_TOKS_TYPE:
            ...
            if(!width && ('c' == *format || 'C' == *format) && *dst_buff != format[1])
                width = 1;
            ...
            parg_target = (int *)va_arg(arg_ptr, int *);
            ...
            case 's':
            case 'S':
                parg_target = (void *)va_arg(arg_ptr, int *);
                if(width && 0 != (parg_target))
                {
